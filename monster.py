...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*👹 KING FREE DDOS GROUP 👹*\n\n"
        "*Use /attack <ip> <port> <duration>*\n"
        "* DM TO BUY :- @Ak_mods_owner *"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users or get_remaining_approval_time(user_id) == "Expired":
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **BSDK APPROVAL LALA @Ak_mods_owner** .*", parse_mode='Markdown')
        return
    ...
    await context.bot.send_message(chat_id=chat_id, text=(
        ...
        f"*🔥 Join :- https://t.me/+zsVBa_w6nHMyYzVl 💥*"
    ), parse_mode='Markdown')
